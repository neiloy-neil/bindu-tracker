-- Also update products.updated_at when the trigger advances the stage,
-- so the pipeline board's "Last Update" column reflects daily entry activity.
...
begin
  ...
  select pid,
    coalesce(sum(cut_color_1), 0), coalesce(sum(cut_color_2), 0),
    coalesce(sum(cut_color_3), 0), coalesce(sum(cut_color_4), 0),
  ...
  select coalesce(sum(pe_sending_qty), 0), coalesce(sum(pe_received_qty), 0)
  into p_out, p_in from production_entries where product_id = pid;

  insert into printing (product_id, out_qty, in_qty, status)
  values (pid, p_out, p_in,
    case when p_in > 0 then 'Received from Print' when p_out > 0 then 'Out for Print' else null end)
  on conflict (product_id) do update set
    out_qty = excluded.out_qty, in_qty = excluded.in_qty, status = excluded.status;

  -- 3. SEWING (with auto status)
  select coalesce(sum(swing_out_qty), 0), coalesce(sum(swing_in_qty), 0)
  into s_out, s_in from production_entries where product_id = pid;

  insert into sewing (product_id, out_qty, in_qty, status)
  values (pid, s_out, s_in,
    case when s_in > 0 then 'Completed' when s_out > 0 then 'Ongoing' else null end)
  on conflict (product_id) do update set
    out_qty = excluded.out_qty, in_qty = excluded.in_qty, status = excluded.status;
  ...
  from production_entries where product_id = pid
  on conflict (product_id) do update set
    in_qty = excluded.in_qty, out_qty = excluded.out_qty,
    reject_qty = excluded.reject_qty, alter_qty = excluded.alter_qty,
  ...
  from production_entries where product_id = pid
  on conflict (product_id) do update set received_qty = excluded.received_qty;
  ...
  update products set current_stage = v_stage, updated_at = now() where id = pid;

  if (TG_OP = 'DELETE') then return OLD; else return NEW; end if;
end;
$$ language plpgsql;

-- Backfill updated_at for all linked products
